...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.respondent.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.respondents.store") }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    @foreach(App\Models\Respondent::GANDER_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('gander') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="gander_{{ $key }}" name="gander" value="{{ $key }}" {{ old('gander', '') === (string) $key ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('respondentcategory') ? 'is-invalid' : '' }}" name="respondentcategory_id" id="respondentcategory_id">
                        @foreach($respondentcategories as $id => $respondentcategory)
                            <option value="{{ $id }}" {{ old('respondentcategory_id') == $id ? 'selected' : '' }}>{{ $respondentcategory }}</option>
                        @endforeach
                    </select>
