        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.answers.update", [$answer->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text" name="date" id="date" value="{{ old('date', $answer->date) }}">
                    @if($errors->has('date'))
                        <div class="invalid-feedback">
                            {{ $errors->first('date') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('event') ? 'is-invalid' : '' }}" type="text" name="event" id="event" value="{{ old('event', $answer->event) }}">
                    @if($errors->has('event'))
                        <div class="invalid-feedback">
                            {{ $errors->first('event') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('country') ? 'is-invalid' : '' }}" name="country_id" id="country_id">
                        @foreach($countries as $id => $country)
                            <option value="{{ $id }}" {{ (old('country_id') ? old('country_id') : $answer->country->id ?? '') == $id ? 'selected' : '' }}>{{ $country }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('counties') ? 'is-invalid' : '' }}" name="counties[]" id="counties" multiple>
                        @foreach($counties as $id => $county)
                            <option value="{{ $id }}" {{ (in_array($id, old('counties', [])) || $answer->counties->contains($id)) ? 'selected' : '' }}>{{ $county }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('sub_counties') ? 'is-invalid' : '' }}" name="sub_counties[]" id="sub_counties" multiple>
                        @foreach($sub_counties as $id => $sub_county)
                            <option value="{{ $id }}" {{ (in_array($id, old('sub_counties', [])) || $answer->sub_counties->contains($id)) ? 'selected' : '' }}>{{ $sub_county }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('constituencies') ? 'is-invalid' : '' }}" name="constituencies[]" id="constituencies" multiple>
                        @foreach($constituencies as $id => $constituency)
                            <option value="{{ $id }}" {{ (in_array($id, old('constituencies', [])) || $answer->constituencies->contains($id)) ? 'selected' : '' }}>{{ $constituency }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('wards') ? 'is-invalid' : '' }}" name="wards[]" id="wards" multiple>
                        @foreach($wards as $id => $ward)
                            <option value="{{ $id }}" {{ (in_array($id, old('wards', [])) || $answer->wards->contains($id)) ? 'selected' : '' }}>{{ $ward }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('wards'))
                        <div class="invalid-feedback">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('other_place') ? 'is-invalid' : '' }}" type="text" name="other_place" id="other_place" value="{{ old('other_place', $answer->other_place) }}">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('offices') ? 'is-invalid' : '' }}" name="offices[]" id="offices" multiple>
                        @foreach($offices as $id => $office)
                            <option value="{{ $id }}" {{ (in_array($id, old('offices', [])) || $answer->offices->contains($id)) ? 'selected' : '' }}>{{ $office }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('sources') ? 'is-invalid' : '' }}" name="sources[]" id="sources" multiple>
                        @foreach($sources as $id => $source)
                            <option value="{{ $id }}" {{ (in_array($id, old('sources', [])) || $answer->sources->contains($id)) ? 'selected' : '' }}>{{ $source }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="text" name="url" id="url" value="{{ old('url', $answer->url) }}">
                    @if($errors->has('url'))
                        <div class="invalid-feedback">
                            {{ $errors->first('url') }}
                        </div>
                ...
                {{--<div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('respondent') ? 'is-invalid' : '' }}" name="respondent_id" id="respondent_id" required>
                        @foreach($respondents as $id => $respondent)
                            <option value="{{ $id }}" {{ (old('respondent_id') ? old('respondent_id') : $answer->respondent->id ?? '') == $id ? 'selected' : '' }}>{{ $respondent }}</option>
                        @endforeach
                    </select>
